...
static COMMAND_LINE_ARGUMENT_A args[] = {
...
#else
	{ "gfx", COMMAND_LINE_VALUE_OPTIONAL, "RFX", NULL, NULL, -1, NULL,
	  "RDP8 graphics pipeline (experimental)" },
#endif
	...
	{ "proxy", COMMAND_LINE_VALUE_REQUIRED, "[<proto>://][<user>:<password>@]<host>:<port>", NULL,
	  NULL, -1, NULL,
	  "Proxy settings: override env.var (see also environment variable below). Protocol \"socks5\" "
	  "should be given explicitly where \"http\" is default.  Note: socks proxy is not supported "
	  "by env. variable" },
