                            ...
                            int action)
{
	...
	case PIPE_CANCEL:
		...
		ret =
		    pdev->control_pipe_request(pdev, RequestId, EndpointAddress, &usbd_status, PIPE_CANCEL);
	...
	case PIPE_RESET:
		...
		ret =
		    pdev->control_pipe_request(pdev, RequestId, EndpointAddress, &usbd_status, PIPE_RESET);
                                           ...
                                           UINT32 UsbDevice, int transferDir)
{
	...
	case URB_FUNCTION_ABORT_PIPE: /** 0x0002  */
		...
		error = urb_pipe_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
		                         transferDir, PIPE_CANCEL);
		break;
	...
	case URB_FUNCTION_GET_CURRENT_FRAME_NUMBER: /** 0x0007 */
		...
		error = urb_get_current_frame_number(callback, data + 8, data_sizem - 8, MessageId, udevman,
		                                     UsbDevice, transferDir);
		break;
	...
	case URB_FUNCTION_SET_FEATURE_TO_DEVICE: /** 0x000D */
		...
		error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman,
	...
	case URB_FUNCTION_VENDOR_DEVICE: /** 0x0017 */
		...
		error =
		    urb_control_vendor_or_class_request(callback, data + 8, data_sizem - 8, MessageId,
		                                        udevman, UsbDevice, (0x02 << 5), /* vendor type */
		                                        0x00, transferDir);
		break;
	...
	case URB_FUNCTION_CLASS_DEVICE: /** 0x001A */
		...
		error =
		    urb_control_vendor_or_class_request(callback, data + 8, data_sizem - 8, MessageId,
		                                        udevman, UsbDevice, (0x01 << 5), /* class type */
		                                        0x00, transferDir);
		break;
	...
	case URB_FUNCTION_GET_CONFIGURATION: /** 0x0026 */
		...
		error = urb_control_get_configuration_request(callback, data + 8, data_sizem - 8, MessageId,
		                                              udevman, UsbDevice, transferDir);
		break;
...
void* urbdrc_process_udev_data_transfer(void* arg)
{
	...
	case REGISTER_REQUEST_CALLBACK:
		WLog_DBG(TAG,
		         ...
		         FunctionId);
		error = urbdrc_process_register_request_callback(callback, pBuffer + 8, cbSize - 8, udevman,
		                                                 UsbDevice);
		break;
	...
	case RETRACT_DEVICE:
		WLog_DBG(TAG,
		         ...
		         FunctionId);
		error = urbdrc_process_retract_device_request(pBuffer + 8, cbSize - 8, udevman, UsbDevice);
		break;
