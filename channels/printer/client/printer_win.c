...
static void printer_win_get_printjob_name(char* buf, int size)
{
	...
	tt = time(NULL);
	t = localtime(&tt);
	sprintf_s(buf, size - 1, "FreeRDP Print Job %d%02d%02d%02d%02d%02d", t->tm_year + 1900,
...
static rdpPrinter** printer_win_enum_printers(rdpPrinterDriver* driver)
{
	...
	{
		printers[num_printers++] = printer_win_new_printer(
		    (rdpWinPrinterDriver*)driver, prninfo[i].pPrinterName, prninfo[i].pDriverName, 0);
	}
