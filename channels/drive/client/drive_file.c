...
static BOOL drive_file_init(DRIVE_FILE* file)
{
	...
	if (file->file_handle == INVALID_HANDLE_VALUE)
	{
		...
		{
		case FILE_SUPERSEDE: /* If the file already exists, replace it with the given file. If it
		                        does not, create the given file. */
			CreateDisposition = CREATE_ALWAYS;
			break;

		case FILE_OPEN: /* If the file already exists, open it instead of creating a new file. If it
		                   does not, fail the request and do not create a new file. */
			CreateDisposition = OPEN_EXISTING;
			break;

		case FILE_CREATE: /* If the file already exists, fail the request and do not create or open
		                     the given file. If it does not, create the given file. */
			CreateDisposition = CREATE_NEW;
			break;

		case FILE_OPEN_IF: /* If the file already exists, open it. If it does not, create the given
		                      file. */
		...
		case FILE_OVERWRITE_IF: /* If the file already exists, open it and overwrite it. If it does
...
BOOL drive_file_query_information(DRIVE_FILE* file, UINT32 FsInformationClass, wStream* output)
{
	...
	case FileBasicInformation:
		...
		Stream_Write_UINT32(output,
		                    fileAttributes.ftLastAccessTime.dwHighDateTime); /* LastAccessTime */
		Stream_Write_UINT32(output,
